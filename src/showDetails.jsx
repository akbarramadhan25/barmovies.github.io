...
export default function Movie() {
            ...
            <>
                {actors.map((e) => (
                    <div key={e.id} className='p-4'>
                        <img src={`https://image.tmdb.org/t/p/w185${e.profile_path}`} alt="" className='opacity-80' />
                        <h2 className='text-xl opacity-80 text-white font-medium py-2'>{e.name}</h2>
                        <p className='text-sm opacity-80 text-white'>Karakter : {e.character}</p>
                    </div>
                ))}
            </>
        )
            ...
            <>
                {crews.map( e => (
                    <div key={e.credit_id} className='p-4'>
                        <img src={`https://image.tmdb.org/t/p/w185${e.profile_path}`} alt="" className='first-letter: opacity-80' />
                        <h2 className='text-xl opacity-80 text-white font-medium py-2'>{e.name}</h2>
    ...
    return(
        <>
            <div className='bg-slate-950 relative h-[6000px]'>
            <div className='ucok max-w-full min-h-[140vh] bg-cover bg-center bg-no-repeat' style={{
                ...
                <div className="max-w-full h-[100vh] md:flex flex-wrap justify-center md:items-center gap-10 mx-auto my-auto pt-16">
                    ...
                    <div className="w-[90%] md:w-[60%] py-5 mx-auto md:mx-0 h-[70%] md:ml-0 ">
                        ...
                        <h3 className='text-2xl opacity-80 font-sans text-white pb-10'>{movieDetails.release_date} ~ {gapName}</h3>
                        <p className='text-xl md:text-2xl  italic pb-5 text-white opacity-80'>{movieDetails.tagline}</p>
                        <h2 className='text-3xl text-white font-medium pb-3 opacity-80'>Overview</h2>
                        <p className='text-xl text-white font-medium  opacity-80' >{movieDetails.overview}</p>
                        <h3>{movieDetails.directors}</h3>
                        <div className="flex justify-center w-full gap-8 pt-5">
                            <div className="w-1/2">
                                <h2 className='text-3xl text-white font-medium pb-2 italic opacity-80'>Negara Produksi</h2>
                                <p className='text-xl text-white font-medium pb-4 opacity-80'>{movieDetails.production_countries.map((negara) => negara.name).join(', ')}</p>
                                <h2 className='text-3xl text-white font-medium pb-2 italic opacity-80'>Penggemar</h2>
                                <p className='text-xl text-white font-medium  opacity-80 mb-10'>{movieDetails.popularity}</p>
                            ...
                            <div className="w-1/2">
                                <h2 className='text-3xl text-white font-medium pb-2 italic opacity-80' >Perusahaan Produksi</h2>
                                <p className='text-xl text-white font-medium opacity-80'>{movieDetails.production_companies.map((company) => company.name).join(', ')}</p>
                            </div>
                        </div>
                    </div>
                </div>
